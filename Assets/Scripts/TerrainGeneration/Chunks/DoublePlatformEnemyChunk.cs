...
public class DoublePlatformEnemyChunk : MonoBehaviour {
	...
	public static Vector3 Generate(Vector3 start, float width, float height)
	{
		...
		int enemyHeight = Random.Range(0,4)*11;
		Instantiate (terrainManager.getRandomEnemy(), new Vector3 ((start.x + end.x)/2, start.y+ enemyHeight + 5, 0), new Quaternion (0, 0, 0, 0));
